...
def login():
	...
	print(f"{m} [LOGIN]")
	print("")
	print(f"{p} [•]{h} Token : {p}https://bit.ly/2Q7Vzre")
	bser = input(f" {p}[•] {b}Username : ")
	basw = input(f" {p}[•] {b}Password : ")
	if bser ==user and basw ==pasw:
		print(f"{p} [!]{h} Login Success")
		os.system("xdg-open https://instagram.com/nano._.emwe")
		time.sleep(5)
		info()
	else:
		print(f"{p} [!]{m} Login Denied")
		time.sleep(2)
		login()
...
def info():
	...
	banner()
	print("")
	print(f"{p} [•]{k} Nama author : {c}Emwe")
	print(f"{p} [•]{k} No Whatsapp : {p}085956067789")
	print(f"{p} [•]{k} Instagram   : {b}nano._.emwe")
	print(f"{k}")
	time.sleep(5)
	nomor()
...
def nomor():
	while True:
		try:
			print(k+'')
			nama = input(" [•] Masukan nama korban : ")
			a = input(" [•] Masukan target [ex: 08xxxx] : ")
			asu=a[0:2]
			if a in(""," "):print(f" {p}[!] {m}Jangan kosong")
			elif "08" not in asu:print(f" {p}[!] {m}Gunakan nomor 08xxx")
			elif len(a)<=10:print(f" {p}[!] {m}Nomor harus lebih dari 10 angka")
			elif a=="085956067789":print(f" {p}[!] {m}Ciee mau spam gw")
			else:
				try:
					suu=int(input(" [•] Masukan jumlah : "))
				except:
					print(f"{p} [!] {m}Masukan Format Angka Jangan Huruf!")
					nomor()
				time.sleep(2)
				print(f"\n {p}[•] {h}Proses...")
				b=a[1:12] 
		...
		except (KeyboardInterrupt,EOFError):exit(" {p}[!] {c}Exit")
...
def banner():
        ...
        print ('')
        print (p+" [+]------------------------------------[+]")
        print (p+"  |"+m+"      Author   : 3mweCyb3r            "+p+"|")
        print (p+"  |"+u+"      Instagram: emwe._.cyber._.56    "+p+"|")
        print (p+"  |"+u+"      Tools    : Spam                "+p+" |")
        print (p+" [+]------------------------------------[+]")

#Looping
def loop():
	print(f" {k}Selesai bang")
	time.sleep(2)
	pilih=input(f"{c} Ingin spam lagi? {p}[{h}y{k}/{m}t{p}] {k}: ")
	if(pilih=="y"):
		nomor()
	elif(pilih=="t"):
		time.sleep(1)
		print(f" {h}Terima kasih!")
		os.system('exit')
	else:
		time.sleep(1)
		print(f"{p} [!] {m}Masukan yang benar!")
		loop()
